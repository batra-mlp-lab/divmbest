...
classdef SegmBrowser    
    ...
    methods
        ...
        function [overlap] = get_overlaps_img(obj, img_name)
        ...
        function [overlap] = get_overlaps_bbox_det(obj, whole_ids)
          ...
          if(exist(cache,'file'))
            ...
            for i=1:length(gtids)
              rec=PASreadrecord(sprintf(obj.VOCopts.annopath,gtids{i}));
        ...
        function masks = get_img_masks(obj, img_id)
          var = load([obj.exp_dir 'MySegmentsMat/' obj.mask_type '/' obj.img_names{img_id} '.mat'], 'masks');
          masks = var.masks;
        end
        
        %%%% Visualization %%%%

        function show_objects(obj, obj_ids)
            just_bbox = true;
        ...
        function show_wholes(obj, whole_ids, titles)          
          DefaultVal('*titles', '[]');
          masks_wholes_to_show = obj.get_masks(whole_ids, 'whole')
          Imgs = obj.get_Imgs(obj.img_names(obj.whole_2_img_ids(whole_ids)));
            
          subplot_auto_transparent_multiple_imgs(masks_wholes_to_show, Imgs, titles);
        end
        ...
        function voc_segm_visuals(obj, png_folder, jpeg_folder, name, plot_labels, solnumber,lambda)  
            DefaultVal('*plot_labels', 'true');
            DefaultVal('*solnumber', '[]');
            DefaultVal('*lambda','[]');
            if ~isempty(lambda),
              dir = [obj.exp_dir '/visuals/' name '_visuals/' sprintf('divsol_lambda_%s/',num2str(lambda))];
            else
            ...
            for i=1:numel(obj.img_names)
                if ~isempty(solnumber)
                  Ipng = imread(sprintf('%s%s/%s_S%04d.png', png_folder, obj.img_names{i}, obj.img_names{i},solnumber));
                else
                ...
                if(plot_labels)
                    ...
                    SvmSegm_plot_labels_at_segms(masks, obj.categories(labels));
                end
                %duh = getframe();
                if ~isempty(solnumber),
                  divsoldir = sprintf('%s%s',dir,obj.img_names{i});
                  if (~exist(divsoldir,'dir'))
                    mkdir(divsoldir);
                  end
                  %imwrite(duh.cdata, sprintf('%s%s/%s_S%04d.png',dir, obj.img_names{i},obj.img_names{i},solnumber));
                  print(gcf,'-dpng','-r300','-painters',sprintf('%s%s/%s_S%04d.png',dir, obj.img_names{i},obj.img_names{i},solnumber));
                else
                  %imwrite(duh.cdata, [dir obj.img_names{i} '.png']);
                  print(gcf,'-dpng','-r300','-painters',[dir obj.img_names{i} '.png']);
                end
            end
        end
        ...
        function [ap,class_fp, class_tp] = voc_detection_score(obj, local_ids, global_ids, labels, scores)          
          ...
          bbox_overlaps = obj.get_overlaps_bbox_det(1:numel(obj.img_names));
        ...
        function Imgs = voc_segm_outputs(obj, img_ids, segm_ids, labels, scores, name, nofiles, algo, solnumber, savepixelwisescrs, origscores, backthreshval,lambda)                    
            % paste detected segments on the image
            
            DefaultVal('*nofiles', 'false');
            DefaultVal('*algo', '''simple''');
            DefaultVal('*solnumber', '[]');
            DefaultVal('*savepixelwisescrs','false');
            DefaultVal('*origscores','[]');
            DefaultVal('*bachthreshval','[]');
            DefaultVal('*lambda','[]');
            
            if savepixelwisescrs,
              if ~isempty(origscores),
                whole_ids = 1:numel(obj.whole_2_img_ids);
                n = hist(obj.whole_2_img_ids(whole_ids), numel(obj.img_names));
                origscores_cell = mat2cell(origscores, numel(obj.categories), n);
              else
                error('Original score matrix empty');
              end
            end
            ...
            %for i=1:numel(img_ids)
                ...
                seg_scores =[];
                if savepixelwisescrs,
                  pixel_scores = ones(size(masks,1), size(masks,2))*backthreshval;
                end
                ...
                if(strcmp(algo, 'simple'))                
                    ...
                    if savepixelwisescrs,
                      for j=1:numel(ids)
                        pixel_scores(masks(:,:,local_segm_ids(ids(j)))) = origscores_cell{i}(labels{i}(ids(j)),local_segm_ids(ids(j)));
                      end
                      os_indx = sub2ind(size(origscores_cell{i}),labels{i},local_segm_ids');
                      seg_scores = origscores_cell{i}(os_indx);
                    end
                else
                ...
                if(nofiles)                    
                    ...
                    if isempty(solnumber),
                      ...
                      dir_name = sprintf([obj.VOCopts.seg.divsolresdir '/%s'],name,obj.VOCopts.testset,sprintf('lambda_%s',num2str(lambda)),obj.img_names{img_ids(i)});
                      resfile = sprintf(obj.VOCopts.seg.divsolrespath,name,obj.VOCopts.testset,sprintf('lambda_%s',num2str(lambda)),obj.img_names{img_ids(i)},obj.img_names{img_ids(i)},solnumber);
                    end
                    ...
                    if savepixelwisescrs,
                      scorefile = sprintf(obj.VOCopts.seg.divsolscrspath,name,obj.VOCopts.testset,sprintf('lambda_%s',num2str(lambda)),obj.img_names{img_ids(i)},obj.img_names{img_ids(i)},solnumber);
                      img_name = obj.img_names{img_ids(i)};
                      seg_labels = labels{i};
                      obj.voc_save_scores(scorefile, seg_scores, local_segm_ids, seg_labels, solnumber, img_name, pixel_scores,backthreshval);
                    end
                end
            end
        end
        function voc_save_scores(obj, scorefile, seg_scores,local_segm_ids,seg_labels,solnumber, img_name, pixel_scores,backthreshval)
          save(scorefile, 'seg_scores','local_segm_ids','seg_labels','solnumber', 'img_name', 'pixel_scores','backthreshval')
        end
